...
todos:
  ...
  - id: subtract-tests
    content: "Write failing tests for tinycalc.subtract on ints and floats"
    status: pending
    files: [tests/test_subtract.py]
  - id: parse-impl
    content: "Implement tinycalc.parse and export it from the package"
    status: pending
    blocked_by: [parse-tests]
    files: [tinycalc/parse.py, tinycalc/__init__.py]
  - id: subtract-impl
    content: "Implement tinycalc.subtract"
    status: pending
    blocked_by: [subtract-tests]
...
2. `tinycalc.subtract(3, 1)` returns 2.
```
...
All files above must exist before the implement protocol runs. Treat this message as my confirmation for the track selection, and use the current branch for git isolation. Do not write code, run tests, or commit: stop as soon as you have told me what happens next and what, if anything, you need me to decide.
