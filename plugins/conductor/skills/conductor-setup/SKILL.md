...
**Setup-specific:** Line 1 = current step and what to do now (answer the pending **User Prompt Protocol** prompt, or the command to run). State "Step N of 7" when resuming. No welcome preambles.
...
3.  **Audit Artifacts (deterministic):** From the project root, run the resume script with the Bash tool:

    ```sh
    python3 "${CLAUDE_PLUGIN_ROOT}/skills/conductor-setup/scripts/resume.py"
    ```
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            ...
            -   **2.1 File Size and Relevance Triage:**
                1.  **Respect Ignore Files:** Before scanning any files, check for `.gitignore` and any project ignore files. Use their patterns to exclude files and directories from your analysis. This avoids token-heavy, irrelevant files like `node_modules`.
                ...
                3.  **Fallback to Manual Ignores:** ONLY if no ignore files exist, fall back to manually ignoring common directories. Example command: `ls -lR -I 'node_modules' -I '.m2' -I 'build' -I 'dist' -I 'bin' -I 'target' -I '.git' -I '.idea' -I '.vscode'`.
...
5.  **Inquire about Project Goal (for Greenfield):**
    -   **Ask the user the following question using the **User Prompt Protocol** and wait for their response before proceeding to the next step:**
        - **header:** "Project Goal"
        - **type:** "text"
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single **User Prompt Protocol** tool call to gather detailed requirements (e.g., target users, goals, features).
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single **User Prompt Protocol** tool call to gather detailed preferences.
        -   **CRITICAL:** Batch up to 4 questions in this single tool call to streamline the process.
...
2.  **Determine Mode:**
    -   **FOR GREENFIELD PROJECTS:** Use the **User Prompt Protocol** to choose the workflow.
        - **questions:**
            - **header:** "Tech Stack"
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, you MUST ask the user for confirmation using the **User Prompt Protocol**:
            - **questions:**
                - **header:** "Tech Stack"
                - **question:** "Is the inferred tech stack (listed above) correct?"
                - **type:** "yesno"
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately use the **User Prompt Protocol** with `type: "text"` to allow the user to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2.  **Select Code Style Guides:**
    -   List the available style guides in the templates directory (resolve per **Plugin Template Path** above): run `ls "${CLAUDE_PLUGIN_ROOT}/templates/code_styleguides/"` with the Bash tool, or use Glob.
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **Determine Mode:** Use the **User Prompt Protocol**:
            - **questions:**
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Action:** Announce "I'll present the available guides in groups. Please select all that apply." Then, immediately use the **User Prompt Protocol** with the batched questions (do not list the questions in the chat).
            -   **Single Tool Call:** Create one **User Prompt Protocol** call containing a `questions` array with one question per group.
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **If user chose "Add More":**
            -   **Action:** Announce "I'll present the additional guides. Please select all that apply." Then, immediately use the **User Prompt Protocol** (do not list the questions in the chat).
            -   **Method:** Use a single **User Prompt Protocol** tool call. Dynamically split the available guides into batches of 4 options max. Create one `multiSelect: true` question for each batch.

3.  **Action:** Construct and execute a command to create the directory and copy all selected files from the resolved templates directory. For example: `mkdir -p conductor/context/code_styleguides && cp "${CLAUDE_PLUGIN_ROOT}/templates/code_styleguides/python.md" "${CLAUDE_PLUGIN_ROOT}/templates/code_styleguides/javascript.md" conductor/context/code_styleguides/`
4.  **Continue:** Immediately proceed to the next section.

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Copy `${CLAUDE_PLUGIN_ROOT}/templates/workflow.md` (resolved per **Plugin Template Path**) to `conductor/context/workflow.md`.
2.  **Determine Mode:** Use the **User Prompt Protocol** to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Workflow"
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Customize":**
        ...
        b. **Final Tweak (Second Batch):** Once the first batch is answered, immediately use a second **User Prompt Protocol** tool call to show the result and allow for any additional tweaks:
            - **questions:**
                - **header:** "Workflow"
                - **type:** "text"
                - **question:**
...
3.  **Determine Mode:** Use the **User Prompt Protocol** to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Product Reqs"
...
5.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single **User Prompt Protocol** tool call to gather detailed requirements.
...
7.  **User Confirmation Loop:**
    ...
    -   **Ask for Approval:** Use the **User Prompt Protocol** to request confirmation. You MUST embed the drafted requirements directly into the `question` field so the user can review them.
        - **questions:**
            - **header:** "Review"
            - **question:**
...
3.  **Confirm Proposal:** Use the **User Prompt Protocol** to validate the proposal:
    - **questions:**
