...
async function validateComponentFrontmatter(pluginDir, pluginName) {
  ...
  if (await pathExists(commandsDir)) {
    ...
    for (const file of files) {
      ...
      // Commands are named by filename and frontmatter is optional — only flag it when present but malformed.
      const content = await fs.readFile(file, "utf8");
      if (normalizeNewlines(content).startsWith("---\n") && !parseFrontmatter(content)) {
        addError(
          `${pluginName}: command file has malformed YAML frontmatter: ${path.relative(repoRoot, file)}`
        );
      }
    }
  }
...
async function validateTemplateReferences(pluginDir, pluginName) {
  // Matches ${CLAUDE_PLUGIN_ROOT}/... references and bare `templates/...` references in skill prose.
  const referencePatterns = [
    /\$\{CLAUDE_PLUGIN_ROOT\}\/([^\s`'")\]]+)/g,
    /`(templates\/[^\s`'")\]]+)`/g,
  ];
  for (const file of await walkFiles(pluginDir)) {
    if (path.extname(file).toLowerCase() !== ".md") {
      continue;
    }
    const content = await fs.readFile(file, "utf8");
    for (const match of referencePatterns.flatMap((pattern) => [...content.matchAll(pattern)])) {
      const reference = match[1].replace(/[.,;:*]+$/, "").replace(/\/+$/, "");
      // Skip placeholder paths (<track_id> etc.) and references into sibling plugins.
      if (!reference || reference.includes("<") || reference.startsWith("..")) {
        continue;
      }
      if (!(await pathExists(path.join(pluginDir, reference)))) {
        addError(
          `${pluginName}: ${path.relative(repoRoot, file)} references missing plugin file "${reference}".`
        );
      }
    }
  }
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    if (entry.source && typeof entry.source === "object" && !Array.isArray(entry.source)) {
      // Remote source (github/git/url object form) — nothing local to validate.
      if (typeof entry.source.source !== "string" || entry.source.source.length === 0) {
        addError(`${label}.source object must include a "source" type string.`);
      }
      continue;
    }
    if (typeof entry.source === "string" && /^(?:https?|git|ssh):\/\//.test(entry.source)) {
      // Remote URL source — nothing local to validate.
      continue;
    }

    const sourcePath = resolveMarketplaceSource(entry.source, pluginRoot ?? "");
    if (!sourcePath) {
      addError(`${label}.source must be a relative path, URL, or source object.`);
      continue;
    }
    ...
    const manifestFields = ["logo", "skills", "agents", "commands", "hooks", "mcpServers"];
    ...
    await validateTemplateReferences(pluginDir, entry.name);
  }
